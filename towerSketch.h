// retrieved from https://github.com/SketchINT-code/SketchINT/blob/main/CPU_src/perflow_test/tower/tower.h

#ifndef _TOWERSKETCH_H
#define _TOWERSKETCH_H
...
public:
    int tot_packets;
    TowerSketch() {}
    TowerSketch(int memory, uint32_t h, int _opt, int _threshold = 0)
    {
        ...
        init(width, h, cs_tower, _opt, _threshold);
    }
    ~TowerSketch() { clear(); }
    ...
    virtual void Insert(char *key, uint16_t key_len, uint32_t row_id = 0, uint32_t f = 1)
    {
        int pre_freq = 0;
        if (opt)
        {
     ...
     */
    void task_record(char *key, uint16_t key_len, uint32_t row_id, int pre_freq)
    {
        if (opt == 1)
    ...
    uint32_t Query(char *key, uint16_t key_len, uint32_t row_id = 0)
    {
...
public:
    TowerSketchCU() {}
    TowerSketchCU(int memory, uint32_t h, int _opt, int _threshold = 0) : TowerSketch(memory, h, _opt, _threshold) {}
